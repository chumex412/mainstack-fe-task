...
const Dropdown = memo(function Dropdown({
  ...
  highlightedIndex,
  highlightOption
}: DropdownType<(e: SyntheticEvent, idx: number) => void>) {
  ...
  return (
    <div
      className={`ds-select__overylay absolute left-0 top-[${overlayTop}px] z-[10] w-full rounded-lg border border-solid bg-white shadow transition-opacity duration-500 ${
            ...
            <>
              {options.map((option, indx) => {
                ...
                return (
                  <>
                    <li
                      className={`ds-select__option flex items-center ${
                        isSelected ? "ds-select__option--selected" : "text-dark"
                      } ${isHighlighted ? "ds-select__option--highlighted" : ""} ${type === "select" ? "gap-x-3 p-3.5" : ""}  text-sm`}
                      key={indx + 1}
                      role="menuitemradio"
